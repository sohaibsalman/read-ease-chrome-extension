...
function createPageCM() {
  ...
  pageCMId = chrome.contextMenus.create(
    {
      title: 'View this page using Read Ease',
      id: 'pageCM',
      contexts: ['page'],
    },
...
function createLinkCM() {
  // Create an entry to allow user to open a given link using Read Ease
  linkCMId = chrome.contextMenus.create(
    {
      title: 'View the linked page using Read Ease',
      id: 'linkCM',
      contexts: ['link'],
    },
    chrome.runtime.lastError
...
function createAutorunCM() {
  ...
  autorunCMId = chrome.contextMenus.create(
    {
      title: "Add this site to Read Ease's auto-run list",
      id: 'autorunCM',
      contexts: ['page'],
    },
    chrome.runtime.lastError
        ...
        chrome.storage.sync.set(
          ...
          function () {
            if (currentDomains.indexOf(url.hostname)) {
              console.log(
                'Read Ease auto-run entry added.\n\nWarning: An auto-run entry with the same hostname has already been added. Be careful to not add two duplicates.'
              );
            } else {
              console.log('Read Ease auto-run entry added.');
            }
          }
        );
      } else {
        console.error(
          "Entry already exists inside of Read Ease's auto-run list. Not adding new entry."
        );
      }
    } else {
